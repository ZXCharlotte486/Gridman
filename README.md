...
古立特（Gridman），财税领域 AI 助手。一个入口覆盖中国会计准则（CAS）、税法、审计、投行估值、内控、内审、ESG、政府会计、财务造假识别、经济法、财务BP等全领域，**35 个专业知识模块 + 45 个 MCP 计算工具 + 18 个端到端工作流**。
...
## MCP 工具一览（44 个业务工具 + 1 个 meta，按 6 类分组）

| 分组 | 工具 |
|------|------|
| 数据处理（8） | 科目余额表处理 · 地址拆分 · 名称模糊匹配 · 数据概览 / 分组 / 筛选 / 透视 · 图表生成 |
| 文档处理（3） | OCR 识别 · PDF 页面操作 · 凭证 PDF 自动拆分 |
| 底稿编制（13） | 银行调节表 · 账龄分析 · 本福特检验 · 折旧重算 · 审计抽样 · 审计调整三栏 TB · 截止测试 · 函证生成 · 底稿初始化 · 重要性计算 · 分析性程序 · 往来重分类 · 凭证巡检 |
| 财务分析（9） | 现金流量表（间接法）· 现金流量表（直接法）· 非经常性损益 · 财务比率 · 纳税调整 · PVM 量价差五效应分解 · 单位指标变动归因 · 多维利润质量诊断 · HHI 集中度 |
| 市场数据（6） | 多市场实时行情 · 历史 K 线 · A 股财务报表 · 公告 / 年报下载 · 企业模糊搜索 · 企业工商 + 风险画像 |
| 办公协同（5，仅 Windows） | 检测应用 · Excel 读 / 写 / 公式 · Word 读 / 追加 · 另存为 · PPT 信息 / 加幻灯片 |
